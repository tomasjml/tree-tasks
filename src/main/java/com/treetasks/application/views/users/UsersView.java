package com.treetasks.application.views.users;

import java.util.Optional;

import com.treetasks.application.data.entity.User;
import com.treetasks.application.data.service.UsersService;
...
public class UsersView extends Div {
    ...
    public UsersView(@Autowired UsersService usersService) {
        setId("users-view");
        // Create UI
        ...
        grid.addColumn("password").setAutoWidth(true).setVisible(true);
        grid.addColumn(user -> {
            return (Integer.toString(user.getLenPassword()));
        }).setHeader("Password Length").setAutoWidth(true);
        TemplateRenderer<User> activeRenderer = TemplateRenderer.<User>of(
        ...
        grid.setDataProvider(new CrudServiceDataProvider<>(usersService));
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                Optional<User> usersFromBackend = usersService.get(event.getValue().getId());
    ...
    private void createEditorLayout(SplitLayout splitLayout) {
        ...
        Component[] fields = new Component[]{username, firstName, lastName, email, phone, dateOfBirth, dateOfCreation, password,
                active};
